...
struct DeliveryNode{
    int IntIDX; //intersection ID
    int load; //-1 to initialized, 0 to dropoff, 1 to pickup, 2 to both, 3 to depot
    vector<unsigned> current_load; //include current good
    vector<unsigned> last_path;//recording path from this node to last node
    
    DeliveryNode(int ID, int typeofint, vector<unsigned> Load, vector<unsigned> Path){
        IntIDX = ID;
        load = typeofint;
        current_load = Load;
        last_path = Path;
...
public:
    ...
    vector<unsigned> Inter_2d_array[100][100];
    
    //a pointer to a 2d array of all POIs
    vector<point_info> POI_2d_array[100][100];
    ...
    //id to closest intersection id
    unordered_map<unsigned, unsigned> cloeset_intersec;
