...
dataStructureSingleton::dataStructureSingleton() {
    ...
    LatLon temp= LatLon(0,0);
    first_LatLon_clicked=temp;
    second_LatLon_clicked=temp;
    clicked_intersection_ID=0;
    get_travel_points_from_map_clicks=false;
    ...
    if (num_POI > 50)
        for (unsigned POI_idx = 0; POI_idx < getNumberOfPointsOfInterest(); POI_idx++) {
            ...
            POI_name_to_id.insert(make_pair(current_info.name.c_str(), POI_idx));
            
            //cloeset_intersec.insert(make_pair(POI_idx, find_closest_intersection(getPointOfInterestPosition(POI_idx))));
